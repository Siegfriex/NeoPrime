# NEO GOD 통합 환경 설정 (PowerShell 전용) - 0127 버전
# 파일명: quick_all_0127.ps1
...
$pathsToAdd = @(
    ...
    $GcloudBin,
    $ClaudeBin,
    "$BaseDir\0126\node_modules\.bin",  # 0126 워크스페이스 경로도 추가
    $CursorHome
)
...
# 0126 경로도 추가 (듀플리케이트 워크스페이스 지원)
$otherWorkDir = "$BaseDir\0126"
...
Write-Host "[TARGET] 0127 폴더" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
