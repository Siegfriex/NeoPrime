...
| 탐구과목 세분화 | 물리/화학/생물 등 과목별 분리 | 과목 테이블 구조화 |
| 내신 환산 추출 | 내신입력 → COMPUTE 연결 분석 | 별도 테이블 필요 |
...
| 기준 | 상태 | 비고 |
|:-----|:----:|:-----|
| 모든 대학/학과별 가중치가 JSON으로 추출됨 | ✅ | 18개 대학 완료 |
| 엑셀 수식이 Python 함수로 1:1 변환됨 | ✅ | Row 59 수식 변환 |
| 테스트 케이스 100% 일치 (abs<1e-6 AND rel<1e-9) | ✅ | 7/7 통과 |
